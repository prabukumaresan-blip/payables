...
function calculateMatchScore(row: ParsedPaymentRow, payable: Payable): { score: number; reason: string } {
  ...
  const isExactTotalMatch = Math.abs(row.amount - Number(payable.amount)) < 0.005;
  const isRemainingMatch = Math.abs(row.amount - remaining) < 0.005;
  const isPartialPayment = row.amount < remaining && row.amount > 0.001;

  if (!isExactTotalMatch && !isRemainingMatch && !isPartialPayment) {
  ...
  if (isExactTotalMatch) {
    score += 5;
    reasons.push('Exact amount match');
  } else if (isRemainingMatch) {
    score += 5;
    reasons.push('Remaining balance match');
...
export function generatePaymentExcelFile(
  ...
  uniqueId?: string,
  exportAmounts?: Record<string, number>
  ...
  const totalAmount = selectedPayables.reduce((sum, p) => {
    const amt = exportAmounts && exportAmounts[p.id] !== undefined
      ? exportAmounts[p.id]
