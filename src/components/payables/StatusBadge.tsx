...
export default function StatusBadge({ payable, onUpdate }: StatusBadgeProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setPaidAmountInput(payable.paid_amount ? String(payable.paid_amount) : '');
    }
  }, [isOpen, payable]);
          ...
          <div className="relative w-full max-w-2xl bg-white border border-slate-200 rounded-2xl p-6 shadow-2xl transition-all z-50 flex flex-col max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
              ...
              <div className="md:col-span-2 md:border-r md:border-slate-100 md:pr-6 space-y-4">
                ...
                {/* Payment Date & Paid Amount input if PARTIAL is selected */}
                  ...
                  <div className="pt-3 border-t border-slate-100 space-y-2">
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        step="0.001"
                        min="0.001"
                        max={payable.category_id === 'cat-8' ? undefined : Number(payable.amount)}
                    ...
                    <button
                      disabled={
                        ...
                        (payable.category_id !== 'cat-8' && parseFloat(paidAmountInput) >= Number(payable.amount))
                      }
                      onClick={() => saveStatus('partial', paymentDate, parseFloat(paidAmountInput))}
                      className="w-full rounded bg-indigo-650 py-2 text-xs font-semibold text-white hover:bg-indigo-600 disabled:opacity-50 flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      {loading && <Loader2 className="h-3.5 w-3.5 animate-spin" />}
