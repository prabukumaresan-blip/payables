...
import { Landowner, Payable } from '@/lib/supabase/mockDb';
...
import { parseLandownerImportFile, ParsedLandownerRow } from '@/lib/utils/fileUtils';

function LandownersContent() {
  ...
  const [editingLandowner, setEditingLandowner] = useState<Landowner | null>(null);
  ...
  const [parsedRows, setParsedRows] = useState<ParsedLandownerRow[]>([]);
  ...
  const handleImportFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const rows = await parseLandownerImportFile(file);
      ...
      rows.forEach((row) => {
        if (!row.name || !row.name.trim()) {
          errors[row.rowIndex] = 'Landowner Name is required';
          initialActions[row.rowIndex] = 'skip';
          return;
        }

        if (row.bank_type === 'OTHER_BANK' && (!row.swift_code || !row.swift_code.trim())) {
          errors[row.rowIndex] = 'SWIFT Code is required for OTHER BANK landowners';
          initialActions[row.rowIndex] = 'skip';
          return;
        }

        // Check duplicates against current list of landowners
        const match = landowners.find(l => l.name.toLowerCase().trim() === row.name.toLowerCase().trim());
        if (match) {
          const isIdentical = 
  ...
  const handleExecuteImport = async () => {
    ...
    try {
      ...
      for (const row of parsedRows) {
        ...
        if (action === 'update') {
          const existing = landowners.find(l => l.name.toLowerCase().trim() === row.name.toLowerCase().trim());
          if (existing) {
            await updateLandowner(existing.id, landownerData);
            updatedCount++;
          } else {
            await createLandowner(landownerData);
            importedCount++;
          }
      ...
      console.error('Error importing landowners:', err);
      alert('An error occurred during import. Some landowners may have been imported.');
    } finally {
      setImportLoading(false);
      setImportProgress(null);
    }
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const lList = await getLandowners();
      const pList = await getAllPayables();
      setLandowners(lList);
      setPayables(pList);
    } catch (e) {
      console.error('Error loading landowners data:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Filtered landowners
  const filteredLandowners = landowners.filter(l => {
    const query = searchQuery.toLowerCase();
    return (
      l.name.toLowerCase().includes(query) ||
      (l.contact_person && l.contact_person.toLowerCase().includes(query)) ||
      (l.email && l.email.toLowerCase().includes(query)) ||
      (l.bank_name && l.bank_name.toLowerCase().includes(query)) ||
      (l.account_no && l.account_no.toLowerCase().includes(query))
    );
  });

  // Calculate stats
  const totalLandowners = landowners.length;
  const landownersWithBank = landowners.filter(l => l.account_no && l.bank_name).length;

  // Active landowners are those whose names appear in "Rent" category payables this month
  const currentMonthStart = new Date();
  currentMonthStart.setDate(1);
  currentMonthStart.setHours(0, 0, 0, 0);

  const activeLandownerNames = new Set(
    payables
      .filter(p => {
        // filter for Rent category and current month payables
        if (!p.vendor_name || !p.created_at) return false;
        
        const isRent = p.category?.name === 'Rent';
        const payableDate = new Date(p.created_at);
        return isRent && payableDate >= currentMonthStart;
      })
      .map(p => p.vendor_name)
      .filter(Boolean)
  );

  const activeLandownersCount = landowners.filter(l => activeLandownerNames.has(l.name)).length;

  // Open modal for Create/Edit
  const handleOpenModal = (landowner: Landowner | null = null) => {
    setFormError(null);
    if (landowner) {
      setEditingLandowner(landowner);
      setName(landowner.name);
      setContactPerson(landowner.contact_person || '');
      setEmail(landowner.email || '');
      setPhone(landowner.phone || '');
      setBankName(landowner.bank_name || '');
      setAccountNo(landowner.account_no || '');
      setSwiftCode(landowner.swift_code || '');
      setBankType(landowner.bank_type || 'BANK_MUSCAT');
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      setFormError('Landowner Name is required');
      return;
    }
    ...
    try {
      ...
      if (editingLandowner) {
        await updateLandowner(editingLandowner.id, landownerData);
      } else {
        await createLandowner(landownerData);
      }
      ...
      console.error(err);
      setFormError('Failed to save landowner details. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };

  // Delete landowner handler
  const handleDeleteClick = (landowner: Landowner) => {
    setDeleteTarget(landowner);
  ...
  const executeDelete = async () => {
    ...
    setLoading(true);
    try {
      await deleteLandowner(deleteTarget.id);
      await loadData();
  ...
  return (
    <AppLayout title="Landowner Directory" showMonthSelector={false}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="bg-white p-5 border border-slate-200 rounded-xl shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Landowners</p>
              <p className="text-2xl font-bold text-slate-850 mt-1">{totalLandowners}</p>
            </div>
          </div>
          ...
          <div className="bg-white p-5 border border-slate-200 rounded-xl shadow-sm flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-850 mt-1">{landownersWithBank}</p>
            </div>
          </div>
          ...
          <div className="bg-white p-5 border border-slate-200 rounded-xl shadow-sm flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-850 mt-1">{activeLandownersCount}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          ...
          <div className="relative min-w-[280px] flex-1 md:flex-initial">
            ...
            <input
              ...
              placeholder="Search landowners by name, contact, bank..."
          ...
          <div className="flex items-center flex-wrap gap-2">
            ...
            >
              ...
              <span className="hidden sm:inline">Add Landowner</span>
            </button>
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          {loading ? (
            ...
            <div className="py-20 text-center text-slate-400">
              <p className="text-sm">No landowners found matching your search.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50 text-slate-500 text-[10px] font-bold uppercase tracking-wider">
                    <th className="py-3 px-6">Landowner Name</th>
                ...
                <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                  {filteredLandowners.map((landowner) => (
                    <tr key={landowner.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="py-3.5 px-6 font-semibold text-slate-900 truncate max-w-[200px]" title={landowner.name}>
                        {landowner.name}
                      </td>
                      <td className="py-3.5 px-6">
                        <span className={cn(
                          "inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-semibold border",
                          landowner.bank_type === 'OTHER_BANK' 
                            ? "bg-amber-50 text-amber-700 border-amber-200/50" 
                            : "bg-indigo-50 text-indigo-700 border-indigo-200/50"
                        )}>
                          {landowner.bank_type === 'OTHER_BANK' ? 'OTHER BANK' : 'BANK MUSCAT'}
                        </span>
                      </td>
                      <td className="py-3.5 px-6 text-slate-600">
                        {landowner.contact_person ? (
                      ...
                      <td className="py-3.5 px-6 text-slate-500">
                        <div className="flex flex-col gap-0.5">
                          ...
                          {!landowner.email && !landowner.phone && <span className="text-slate-400">—</span>}
                        </div>
                      </td>
                      <td className="py-3.5 px-6 text-slate-700 font-medium">
                        {landowner.bank_name || <span className="text-slate-400 font-normal">—</span>}
                      </td>
                      <td className="py-3.5 px-6 font-mono text-xs text-slate-800">
                        {landowner.account_no || <span className="text-slate-400 font-normal font-sans">—</span>}
                      </td>
                      <td className="py-3.5 px-6 font-mono text-xs font-semibold text-slate-750">
                        {landowner.swift_code || <span className="text-slate-400 font-normal font-sans">—</span>}
                      </td>
                      <td className="py-3.5 px-6 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            onClick={() => handleOpenModal(landowner)}
                          ...
                          <button
                            onClick={() => handleDeleteClick(landowner)}
            ...
            <div className="relative w-full max-w-lg bg-white border border-slate-200 rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden transition-all duration-200">
              ...
              <div className="px-6 py-5 border-b border-slate-150 flex items-center justify-between shrink-0 bg-slate-50/50">
                <h3 className="text-md font-bold text-slate-900 font-sans">
                  {editingLandowner ? 'Edit Landowner Details' : 'Add New Landowner'}
                </h3>
                <button
              ...
              <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                ...
                {/* Landowner Name & Bank Type */}
                <div className="grid grid-cols-3 gap-4">
                  <div className="col-span-2 space-y-1.5">
                    <label className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                      Landowner Name <span className="text-rose-500">*</span>
                    </label>
                    <input
                      type="text"
                      required
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="e.g. Hilal Al Balushi"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      placeholder="e.g. landowner@example.com"
                      className="w-full rounded-lg border border-slate-200 bg-white py-2 px-3 text-sm text-slate-855 outline-none focus:border-indigo-500"
                    />
                  </div>
                  <div className="space-y-1.5">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                    Are you sure you want to delete landowner &quot;{deleteTarget.name}&quot;? This action cannot be undone.
                  </p>
                </div>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-4xl bg-white border border-slate-200 rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden transition-all duration-200">
              ...
              <div className="px-6 py-5 border-b border-slate-150 flex items-center justify-between shrink-0 bg-slate-50/50">
                <div>
                  <h3 className="text-md font-bold text-slate-900 font-sans">
                    Import Landowners from Excel
                  </h3>
                  <p className="text-xs text-slate-500 mt-1">
                    Upload an Excel spreadsheet with landowner contact and bank details.
                  </p>
                </div>
                <button
                    ...
                    <div className="bg-indigo-50/50 border border-indigo-100 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-indigo-700">
                          Format columns in your sheet as: Name, Bank Type (BANK_MUSCAT / OTHER_BANK), Contact Person, Email, Phone, Bank Name, Account Number, SWIFT Code.
                        </p>
                      </div>
                    </div>
                          ...
                          {parsedRows.map((row) => {
                            ...
                            return (
                              <tr key={row.rowIndex} className={cn("hover:bg-slate-50/50", error ? "bg-rose-50/20" : "")}>
                                ...
                                <td className="py-2 px-4 text-slate-500">{row.contact_person || '—'}</td>
                                <td className="py-2 px-4">
                                  <span className={cn(
                  ...
                  <div className="flex flex-col items-center justify-center p-8 text-center space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto leading-relaxed">
                        Landowner directory database has been updated with the changes from your Excel spreadsheet.
                      </p>
                    </div>
