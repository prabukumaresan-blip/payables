      ...
      if (muscatPayables.length > 0) {
        ...
        for (const payable of muscatPayables) {
          await updatePayable(payable.id, {
            ...
            ...(markPaidAfterExport ? {
              status: 'paid',
              payment_date: payable.status !== 'paid' ? today : payable.payment_date,
              paid_amount: payable.status !== 'paid' ? payable.amount : payable.paid_amount
            } : {})
          });
        }
      }
            ...
            <div className="relative w-full max-w-lg bg-white border border-slate-200 rounded-2xl shadow-2xl flex flex-col overflow-hidden transition-all duration-200">
              ...
              <div className="px-6 py-5 space-y-4 text-slate-700">
                <div className={cn(
                  ...
                  You have selected <span className="font-bold">{selectedIds.length} payable(s)</span> for a total of <span className="font-bold">{formatOMR(payables.filter(p => selectedIds.includes(p.id)).reduce((sum, p) => sum + (p.amount - (p.paid_amount || 0)), 0))}</span>.
                  {isMuscatSelected && isOtherBankSelected ? (
                    ...
                    {payables.filter(p => selectedIds.includes(p.id)).map((payable) => (
                      <div key={payable.id} className="flex items-center gap-3 py-1 border-b border-slate-50 last:border-b-0">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] text-slate-400">
                            Amount: {formatOMR(payable.amount - (payable.paid_amount || 0))}
                          </p>
                        </div>
                        <input
                          type="text"
